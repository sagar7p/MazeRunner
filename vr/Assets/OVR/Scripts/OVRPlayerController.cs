...
public class OVRPlayerController : MonoBehaviour
{
	...
	public static float myTimer   		  = 300.0f;
	...
	{
		if (myTimer > 0) // has the level been completed
		{
			myTimer -= Time.deltaTime; // I need timer which from a particular time goes to zero
		}
		else {
			myTimer = 300.0f;
	...
	public virtual void UpdateMovement()
	{
		...
		if(HaltUpdateMovement == true)
			return;
		if(Application.loadedLevel != 0) moveForward = false;
		bool moveLeft  	 = false;
	...
	public bool Jump()
	{
		...
		MoveThrottle += new Vector3(0, 3, 0);
